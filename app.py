...
def calculate_site_metrics(_processed_df, ordered_stages, ts_col_map):
    ...
    try:
        ...
        for site_name, group in site_groups:
            ...
            metrics['Site Screen Fail %'] = (count_sf / count_icf) if count_icf > 0 else 0.0 # Changed from count_pof to count_icf as denominator for Screen Fail %
...
def score_sites(_site_metrics_df, weights):
    ...
    try:
        ...
        if not scaled_metrics_data.empty:
            for col in metrics_to_scale:
                 if col in scaled_metrics_data.columns:
                     ...
                     else: scaled_metrics_display[col] = 0.5 # Default to 0.5 if scaling fails (e.g. all NaNs after fill)
        ...
        for metric, weight_value in weights.items():
             if metric in scaled_metrics_display.columns:
                 ...
                 if current_scaled_metric_series is not None:
                     site_metrics_df_indexed['Score_Raw'] += current_scaled_metric_series.fillna(0.5) * weight_value # fillna(0.5) for any remaining NaNs
                 else: site_metrics_df_indexed['Score_Raw'] += 0.5 * weight_value # Should not happen if metric in columns
        ...
        if len(site_metrics_df_indexed) > 1:
            ...
            try: site_metrics_df_indexed['Grade'] = pd.qcut(site_metrics_df_indexed['Score_Rank_Percentile'], q=bins, labels=labels, duplicates='drop')
            except ValueError: # Handles cases with insufficient distinct values for qcut
                 ...
                 def assign_grade_fallback(score_value):
                     ...
                     score_value = round(score_value) # Round to nearest int for grading bands
        ...
        elif len(site_metrics_df_indexed) == 1: # Single site
            def assign_single_site_grade(score_value): # Fallback grading for single site
                if pd.isna(score_value): return 'N/A'
            ...
            site_metrics_df_indexed['Grade'] = None # Or pd.Series(dtype=str)
        ...
        if _site_metrics_df is not None and not _site_metrics_df.empty: # Attempt to return original data if error
    ...
    try:
        ...
        for rate_key in manual_rates_sidebar.keys():
            ...
            if actual_col_from and col_to_cleaned_name and actual_col_from in hist_counts.columns and col_to_cleaned_name in hist_counts.columns:
                ...
                for month_period_loop in hist_counts.index:
                    if month_period_loop.end_time + pd.Timedelta(days=maturity_days_for_this_rate) < pd.Timestamp(datetime.now()):
                        ...
                        if denominator_val < MIN_DENOMINATOR_FOR_RATE_CALC:
                            ...
                            is_manual_rate_placeholder = (manual_rate_for_key >= 0.99 or manual_rate_for_key <= 0.01) # Check if manual rate is a likely placeholder
                ...
                else: # Window is 0 or less (e.g. overall average requested with 999 and no mature data) or no data
                    if not adjusted_monthly_rates_series.empty: # If there's any mature data, use its mean
...
def calculate_projections(_processed_df, ordered_stages, ts_col_map, projection_inputs):
    ...
    if inter_stage_lags:
        for stage_from, stage_to in projection_segments_for_lag_path:
            ...
            if ts_col_map.get(stage_from) and ts_col_map.get(stage_to): # Check if stages exist in funnel def
                if pd.notna(lag_value): calculated_sum_of_lags += lag_value; valid_segments_count += 1
                else: all_segments_found_and_valid = False; break # Missing lag for a defined segment
            # else: all_segments_found_and_valid = False; break # Stage not in funnel definition (should be rare if funnel is consistent)
        ...
        else: # Reset for fallback if not all segments were found or had valid lags
    ...
    try:
        ...
        for month_period in future_months:
            ...
            cpqr_for_month = assumed_cpqr_dict.get(month_period, 120.0) # Default CPQR if not specified
            if cpqr_for_month <= 0: cpqr_for_month = 120.0 # Ensure positive CPQR
        ...
        for stage_from, stage_to in projection_segments_for_lag_path: # Use the defined path
            ...
            if current_stage_count_col in projection_cohorts.columns:
                ...
                projection_cohorts[proj_col_to_name] = proj_counts_for_to_stage.round(0).fillna(0).astype(int) # Round at each step
                current_stage_count_col = proj_col_to_name
            else: projection_cohorts[proj_col_to_name] = 0; current_stage_count_col = proj_col_to_name # Should not happen if logic is tight
            if stage_to == STAGE_SIGNED_ICF: icf_proj_col_name_base = proj_col_to_name; break # Found ICF projection column
        ...
        overall_current_lag_days_to_use = avg_actual_lag_days_for_display; days_in_avg_month = 30.4375 # Consistent average days in month
        for cohort_start_month_iter, row_data in projection_cohorts.iterrows():
            icfs_from_this_cohort_val = row_data[icf_proj_col_name_base] # This is integer ICFs generated by cohort
            ...
            icfs_month_1_val = icfs_from_this_cohort_val * fraction_for_current_offset_month_val # Float ICFs landing in month 1
            icfs_month_2_val = icfs_from_this_cohort_val * fraction_for_next_month_val # Float ICFs landing in month 2
        ...
        if not lpi_month_series_val.empty:
            ...
            if icfs_in_lpi_month_val > 0:
                ...
                lpi_day_offset_val = int(np.ceil(fraction_of_lpi_month_val * days_in_avg_month)); lpi_day_offset_val = max(1, lpi_day_offset_val) # Ensure at least day 1
            ...
            elif icfs_needed_in_lpi_month_val <= 0: # Goal met by end of previous month
                 lpi_date_str = (lpi_month_period_val.start_time - pd.Timedelta(days=1)).strftime('%Y-%m-%d') # End of previous month
            else: lpi_date_str = lpi_month_period_val.start_time.strftime('%Y-%m-%d') # Beginning of LPI month if ICFs in LPI month is 0 but somehow still needed
        
        projection_cohorts['Cumulative_Projected_ICF_Generated'] = projection_cohorts[icf_proj_col_name_base].cumsum()
        ads_off_s_granular = projection_cohorts[projection_cohorts['Cumulative_Projected_ICF_Generated'] >= (goal_total_icfs - 0.5 + 1e-9)] # Use goal_total_icfs (int) with a slight nudge for float comparison
        if not ads_off_s_granular.empty:
            ...
            if icfs_needed_from_ads_off_month_granular <= 0: # Goal met by previous cohort's generation
                ...
                if prev_m_ads_off_granular in projection_cohorts.index and cum_gen_before_ads_off_month_granular >= (goal_total_icfs - 0.5 + 1e-9):
                     ads_off_date_str = prev_m_ads_off_granular.end_time.strftime('%Y-%m-%d') # Ads off at end of previous month
                else: # Goal met exactly at start of current month by prior generation
                     ads_off_date_str = ads_off_month_period_granular.start_time.strftime('%Y-%m-%d')
            elif icfs_gen_by_ads_off_month_granular > 1e-9: # If this cohort generates ICFs
            ...
            else: # This month generates 0 ICFs, but somehow needed from it (should not happen if prev check is fine)
                ads_off_date_str = ads_off_month_period_granular.start_time.strftime('%Y-%m-%d')
        ...
        lag_for_cpicf_display_val = int(np.round(overall_current_lag_days_to_use / days_in_avg_month)) # Lag in whole months for CPICF display alignment
        for i_cohort_idx, cohort_start_month_cpicf in enumerate(projection_cohorts.index):
            ...
            if primary_land_m_cpicf in cpicf_display_mean_val.index:
                # Only assign if not already assigned (to give priority to earlier cohorts if lags cause overlap)
            ...
            if site_performance_data.empty or 'Site' not in site_performance_data.columns:
                ...
                for site_name_s_proj in all_sites_proj:
                    ...
                    for month_period_s_proj in future_months:
                        ...
                        site_data_collector_proj[site_name_s_proj][(month_str_s_proj, 'Projected ICFs Landed')] = 0.0 # Initialize as float
                ...
                for cohort_start_month_s_proj, cohort_row_s_proj in projection_cohorts.iterrows():
                    ...
                    elif len(all_sites_proj) > 0: # Fallback: equal distribution if no historical proportions
                    ...
                    # Round POF allocations at the end to maintain sum
                    current_sum_pof_float = sum(site_pof_allocations_this_cohort.values())
                    if abs(current_sum_pof_float - total_psq_this_cohort_s_proj) > 1e-6 and current_sum_pof_float > 0: # Rescale if sum is off
                    ...
                    # Distribute rounding difference (if any) to the site with largest fraction or largest allocation
                    if diff_after_round != 0 and site_pof_allocations_this_cohort:
                        site_to_adjust = max(site_pof_allocations_this_cohort, key=lambda s: site_pof_allocations_this_cohort[s] - math.floor(site_pof_allocations_this_cohort[s])) # Site with largest fractional part
                    ...
                    for site_name_iter_s_proj in all_sites_proj:
                        ...
                        current_site_proj_count_s_proj = float(site_proj_pof_cohort_s_proj) # Start with float for rate multiplication

                        for i_seg_s, (stage_from_seg_s, stage_to_seg_s) in enumerate(projection_segments_for_lag_path):
                            site_rate_key_s = "" # Map funnel segment to site metric column name
                            ...
                            rate_to_use_s = final_projection_conv_rates.get(overall_rate_key_s, 0.0) # Default to overall rate
                            ...
                            if not site_perf_row_s_proj.empty and site_rate_key_s and site_rate_key_s in site_perf_row_s_proj.columns:
                                ...
                                if pd.notna(site_specific_rate_val_s) and site_specific_rate_val_s >= 0: # Ensure rate is valid
                                    rate_to_use_s = site_specific_rate_val_s
                            
                            current_site_proj_count_s_proj *= rate_to_use_s # Multiply by rate
                            if stage_to_seg_s == STAGE_SIGNED_ICF: break # Reached ICFs for this site cohort
                        
                        site_proj_icfs_generated_this_cohort_s = current_site_proj_count_s_proj # This is float ICFs generated by site for this cohort

                        lag_to_use_for_site_s = overall_current_lag_days_to_use # Default to overall lag
                ...
                if site_data_collector_proj:
                    ...
                    for m_period_fmt_s_proj in future_months.strftime('%Y-%m'):
                        ...
                        if pof_col_tuple in site_level_projections_df_final.columns: # Already int due to rounding earlier
                            site_level_projections_df_final[pof_col_tuple] = site_level_projections_df_final[pof_col_tuple].astype(int)

                    site_level_projections_df_final = site_level_projections_df_final.sort_index(axis=1, level=[0,1]) # Sort columns
...
# --- MODIFIED: AI Forecast Core Function with Frontloading Logic ---
def calculate_ai_forecast_core(
    ...
    site_scoring_weights_for_ai: dict, # Currently unused in core AI logic, but available
    ...
    if not all([processed_df is not None, not processed_df.empty, ordered_stages, ts_col_map,
                effective_projection_conv_rates, site_metrics_df is not None]): # site_metrics_df can be empty if no sites
    ...
    if ai_monthly_ql_capacity_multiplier <=0:
        ai_monthly_ql_capacity_multiplier = 1.0 # Default to 1x if invalid

    ts_pof_col_for_prop = ts_col_map.get(STAGE_PASSED_ONLINE_FORM)

    main_funnel_path_segments = [ # Used to calculate overall POF to ICF rate
    ...
    for segment in main_funnel_path_segments:
        ...
        if rate is None or rate < 0: # Rate must be non-negative
    ...
    if overall_pof_to_icf_rate <= 1e-9: # Effectively zero conversion
    ...
    # Determine current goal LPI and ICF number based on run_mode
    current_goal_icf_number = goal_icf_number_orig # Integer goal for this run
    if run_mode == "primary":
        current_goal_lpi_month_period = pd.Period(goal_lpi_date_dt_orig, freq='M')
    elif run_mode == "best_case_extended_lpi":
        max_possible_proj_end_month_for_lpi = proj_start_month_period + projection_horizon_months -1
        current_goal_lpi_month_period = max_possible_proj_end_month_for_lpi
    else: # Should not happen
        current_goal_lpi_month_period = pd.Period(goal_lpi_date_dt_orig, freq='M')


    days_in_avg_m = 30.4375
    avg_lag_months_approx = int(round(avg_overall_lag_days / days_in_avg_m)) # Approximate lag in whole months

    # Define projection calculation window:
    # End month for calculation should be far enough to see landings from QLs generated to meet LPI
    # and also to allow the best_case_extended_lpi to show results up to its (extended) LPI.
    max_possible_proj_end_month_overall = proj_start_month_period + projection_horizon_months -1
    # Furthest month we might need to see landings or plan QLs for.
    # If LPI is far, go to LPI + buffer. If LPI is close, still need some horizon.
    ...
    # Ensure calc_horizon_end_month_for_display is not before proj_start_month_period
    ...
    if projection_calc_months.empty :
        ...
        if current_goal_lpi_month_period < first_possible_landing_month_check:
             feasibility_details_early = f"Goal LPI ({current_goal_lpi_month_period.strftime('%Y-%m')}) is too soon. Minimum landing month: {first_possible_landing_month_check.strftime('%Y-%m')} (Lag: {avg_overall_lag_days:.1f} days)."
             return default_return_ai[0], default_return_ai[1], default_return_ai[2], feasibility_details_early, True, 0
        return default_return_ai[0], default_return_ai[1], default_return_ai[2], "Projection calculation window is invalid for AI Forecast (LPI vs Horizon issue).", True, 0

    ai_gen_df = pd.DataFrame(index=projection_calc_months) # For QL generation planning
    ai_gen_df['Required_QLs_POF_Initial'] = 0.0 # Theoretical QLs this month aims for
    ai_gen_df['Required_QLs_POF_Final'] = 0.0   # Actual QLs after site cap redistribution
    ai_gen_df['Unallocatable_QLs'] = 0.0      # QLs that couldn't be allocated due to caps
    ai_gen_df['Generated_ICF_Mean'] = 0.0     # Float ICFs generated by this cohort's QLs
    ai_gen_df['Adjusted_CPQL_For_Month'] = estimated_cpql_user
    ai_gen_df['Implied_Ad_Spend'] = 0.0

    icfs_still_to_assign_globally = float(current_goal_icf_number) # Target for this specific run

    # Determine valid months for QL generation
    # QLs generated in 'latest_permissible_generation_month' should land ICFs by 'current_goal_lpi_month_period'
    latest_permissible_generation_month = current_goal_lpi_month_period - avg_lag_months_approx
    earliest_permissible_generation_month = proj_start_month_period # Cannot generate before projection starts

    valid_generation_months_for_planning = pd.period_range(
        start=max(earliest_permissible_generation_month, projection_calc_months.min()), # Ensure within overall calc window
        end=min(latest_permissible_generation_month, projection_calc_months.max())      # And within overall calc window
    )

    # Calculate baseline monthly QL volume for capacity heuristic
    baseline_monthly_ql_volume = 1.0 # Default if no history
    if ts_pof_col_for_prop and ts_pof_col_for_prop in processed_df.columns and not processed_df.empty and 'Submission_Month' in processed_df.columns:
        ...
        if not valid_pof_df_baseline.empty and 'Submission_Month' in valid_pof_df_baseline:
            ...
            if num_unique_hist_months > 0:
                months_for_baseline_calc = min(num_unique_hist_months, 6) # Use up to last 6 unique months
                ...
                if not baseline_data_for_avg.empty:
                    ...
                    calculated_baseline = total_pof_baseline_period / months_for_baseline_calc # Avg QLs per month
    ...
    if monthly_ql_capacity_target_heuristic < 1: monthly_ql_capacity_target_heuristic = 50 # Minimum heuristic if baseline is very low


    # Site-level setup for QL distribution and capping
    site_level_monthly_qlof = {} # Stores QLs allocated per site per generation month
    all_sites_list_ai = np.array([])
    if not site_metrics_df.empty and 'Site' in site_metrics_df.columns:
        all_sites_list_ai = site_metrics_df['Site'].unique()

    hist_site_pof_prop = pd.Series(dtype=float) # Historical POF proportions for sites
    if ts_pof_col_for_prop and ts_pof_col_for_prop in processed_df.columns and 'Site' in processed_df.columns:
        ...
        if not valid_pof_data_for_prop.empty and 'Site' in valid_pof_data_for_prop:
             hist_site_pof_prop = valid_pof_data_for_prop['Site'].value_counts(normalize=True)

    site_redistribution_scores = {} # Used for redistributing QLs if caps are hit
    if not site_metrics_df.empty and 'Site' in site_metrics_df.columns and 'Qual -> ICF %' in site_metrics_df.columns:
        for _, site_row_score in site_metrics_df.iterrows():
            ...
            # Use a tiny floor for score to allow redistribution even if score is 0
            site_redistribution_scores[site_row_score['Site']] = score_val if score_val > 1e-6 else 1e-6


    # --- QL Generation Planning Loop ---
    if not valid_generation_months_for_planning.empty:
        for gen_month in valid_generation_months_for_planning: # Iterate FORWARDS through valid generation months
            if icfs_still_to_assign_globally <= 1e-9: # Goal met in terms of ICF generation
                break

            # Determine QLs needed for remaining ICFs (float)
            ...
            # Determine QL target for this month, limited by heuristic
            ...
            # Refined rounding for QL target for the current month:
            if icfs_still_to_assign_globally > 1e-9 and \
               ...
               qls_theoretically_needed_for_remaining_float > 0:
                # If in the final stretch, not hitting heuristic capacity, aim to precisely generate.
                ...
                # If heuristic-limited target is a small positive fraction.
                current_month_initial_ql_target = math.ceil(ql_target_potential_this_month_heuristic_limited)
            else: # Standard case (heuristic limit hit, or zero needed, or >1).
                current_month_initial_ql_target = round(max(0, ql_target_potential_this_month_heuristic_limited))

            ai_gen_df.loc[gen_month, 'Required_QLs_POF_Initial'] = current_month_initial_ql_target
            site_ql_allocations_month_specific = {site: 0 for site in all_sites_list_ai} # QLs per site for this month
            unallocatable_this_month = 0

            if current_month_initial_ql_target > 0 and all_sites_list_ai.size > 0:
                # Initial distribution based on historical proportions or equal share
                temp_site_allocations_float = {}
                if not hist_site_pof_prop.empty and hist_site_pof_prop.sum() > 1e-9:
                    for site_n_dist, prop_dist in hist_site_pof_prop.items():
                        if site_n_dist in site_ql_allocations_month_specific: # ensure site is in current list
                            temp_site_allocations_float[site_n_dist] = current_month_initial_ql_target * prop_dist
                else: # Fallback to equal distribution if no historical props or no sites in props
                    ql_per_site_fallback_float = current_month_initial_ql_target / all_sites_list_ai.size
                    for site_n_dist in all_sites_list_ai: temp_site_allocations_float[site_n_dist] = ql_per_site_fallback_float

                # Round initial float allocations and adjust sum to match target QLs
                ...
                if diff_ql_rounding_adj != 0 and all_sites_list_ai.size > 0:
                    # Distribute rounding difference, prioritizing sites with larger original float allocations or scores
                    target_site_for_diff_adj = all_sites_list_ai[0] # Fallback
                    if temp_site_allocations_float: # Use largest float allocation
                         target_site_for_diff_adj = max(temp_site_allocations_float, key=temp_site_allocations_float.get, default=all_sites_list_ai[0])
                    elif site_redistribution_scores: # Or best score if float allocs are all zero/equal
                         best_site_cand_adj = max(site_redistribution_scores, key=site_redistribution_scores.get, default=None)
                         if best_site_cand_adj and best_site_cand_adj in site_ql_allocations_month_specific:
                             target_site_for_diff_adj = best_site_cand_adj
                    site_ql_allocations_month_specific[target_site_for_diff_adj] += diff_ql_rounding_adj


                # Site Capping and Redistribution Loop
                max_iterations_site_cap_loop = 10 # Prevent infinite loops
                for iteration_cap_loop in range(max_iterations_site_cap_loop):
                    excess_ql_pool_iter_val = 0; newly_capped_this_iter_val = False
                    # Apply caps
                    for site_n_iter_cap, allocated_qls_iter_cap in site_ql_allocations_month_specific.items():
                        site_cap_val_iter = site_caps_input.get(site_n_iter_cap, float('inf')) # Get cap, default to no cap
                    ...
                    if excess_ql_pool_iter_val < 1: break # No more excess QLs (or fractional)

                    # Redistribute excess
                    candidate_sites_for_rd_list = {
                        s: score for s, score in site_redistribution_scores.items()
                        if s in site_ql_allocations_month_specific and site_ql_allocations_month_specific[s] < site_caps_input.get(s, float('inf'))
                    }
                    if not candidate_sites_for_rd_list: # No sites can take more QLs
                        unallocatable_this_month += round(excess_ql_pool_iter_val); break

                    total_score_candidates_rd = sum(candidate_sites_for_rd_list.values())
                    if total_score_candidates_rd <= 1e-9: # No scores to distribute by
                        unallocatable_this_month += round(excess_ql_pool_iter_val); break

                    temp_excess_after_rd_iter = excess_ql_pool_iter_val # Track remaining excess within this redistribution attempt
                    # Sort candidates by score to prioritize higher-scoring sites for redistribution
                    ...
                    for site_rd_val, score_rd_val in sorted_candidates_for_rd_list:
                        if temp_excess_after_rd_iter < 1: break # All redistributed (or fractional remains)
                        ...
                        actual_add_rounded_val = round(actual_add_to_site_val) # Redistribute whole QLs

                        site_ql_allocations_month_specific[site_rd_val] += actual_add_rounded_val
                        temp_excess_after_rd_iter -= actual_add_rounded_val

                    excess_ql_pool_iter_val = max(0, temp_excess_after_rd_iter) # Update pool with any remaining fraction

                    if excess_ql_pool_iter_val < 1 or not newly_capped_this_iter_val: # If no more significant excess or no sites were newly capped this iteration
                        if excess_ql_pool_iter_val >= 1: unallocatable_this_month += round(excess_ql_pool_iter_val)
                        break
                    if iteration_cap_loop == max_iterations_site_cap_loop - 1 and excess_ql_pool_iter_val >=1 : # Max iterations reached with excess
                        unallocatable_this_month += round(excess_ql_pool_iter_val)
            elif current_month_initial_ql_target > 0 and all_sites_list_ai.size == 0 : # QLs to assign but no sites defined
            ...
            site_level_monthly_qlof[gen_month] = site_ql_allocations_month_specific.copy() # Store site allocations
            ...
            icfs_still_to_assign_globally -= icfs_generated_this_month_float # Decrease remaining ICFs by float value
        ...
        if icfs_still_to_assign_globally > 1e-9:
            ...
            feasibility_details_no_gen = f"Goal LPI ({current_goal_lpi_month_period.strftime('%Y-%m')}) combined with lag ({avg_overall_lag_days:.1f} days) results in no valid QL generation months within the projection window. Minimum landing month for first QLs: {first_possible_landing_month_check_no_gen.strftime('%Y-%m')}."
            if current_goal_lpi_month_period < first_possible_landing_month_check_no_gen:
                 feasibility_details_no_gen = f"Goal LPI ({current_goal_lpi_month_period.strftime('%Y-%m')}) is too soon. Minimum landing month: {first_possible_landing_month_check_no_gen.strftime('%Y-%m')} (Lag: {avg_overall_lag_days:.1f} days)."
            # Prepare empty results for return
    ...
    # After QL planning loop, calculate total generated ICFs (float)
    ...
    # CPQL Inflation and Implied Ad Spend Calculation
    total_unallocated_qls_run = 0 # Sum of unallocatable QLs over all generation months
    for gen_month_spend_idx_val in ai_gen_df.index:
        ...
        if ql_vol_increase_threshold_pct > 0 and cpql_inflation_factor_pct > 0 and baseline_monthly_ql_volume > 0:
            if final_qls_for_cpql_calc_val > baseline_monthly_ql_volume: # If QLs exceed baseline
                ...
                # How many multiples of the threshold were crossed
    ...
    # ICF Variation for CPICF calculation
    ...
    for cohort_g_month_land_idx_val in ai_gen_df.index:
        icfs_gen_this_cohort_land_val = ai_gen_df.loc[cohort_g_month_land_idx_val, 'Generated_ICF_Mean'] # Use float ICFs
        ...
        frac_next_land_val = rem_lag_days_land_val / days_in_avg_m
        frac_curr_land_val = 1.0 - frac_next_land_val
    ...
    ai_results_df['Projected_ICF_Landed'] = ai_results_df['Projected_ICF_Landed'].round(0).astype(int) # Round monthly landed ICFs to integers
    ai_results_df['Cumulative_ICF_Landed'] = ai_results_df['Projected_ICF_Landed'].cumsum()
    # Populate Target_QLs_POF and Implied_Ad_Spend in results_df for display, aligned with landing months (though QLs/Spend are by generation month)
    ...
    # Align CPICF values with landing months for display
    ...
    for g_m_cpicf_idx_val in ai_gen_df.index:
        if ai_gen_df.loc[g_m_cpicf_idx_val, 'Generated_ICF_Mean'] > 0: # If cohort generated ICFs
            display_land_m_cpicf_val = g_m_cpicf_idx_val + avg_lag_months_approx # Align with approximate landing month
            if display_land_m_cpicf_val in cpicf_m_res.index and pd.isna(cpicf_m_res.loc[display_land_m_cpicf_val]): # Assign if not already set
    ...
    # --- Ads Off Date Calculation ---
    ai_gen_df['Cumulative_Generated_ICF_Final'] = ai_gen_df['Generated_ICF_Mean'].cumsum()
    # Find month where cumulative *generated* ICFs (float) meet/exceed goal
    ads_off_s_val = ai_gen_df[ai_gen_df['Cumulative_Generated_ICF_Final'] >= (current_goal_icf_number - 1e-9)] # Compare with float precision
    ...
    if not ads_off_s_val.empty:
        ...
        if icfs_needed_from_ads_off_month_cohort_val <= 0: # Goal met by previous cohort's generation
            ...
            if prev_month_period_ads_off_val in ai_gen_df.index and \
               cumulative_generated_before_ads_off_month_cohort_val >= (current_goal_icf_number - 1e-9) :
                 ...
                 ads_off_date_str_calc_val = ads_off_month_period_val_calc.start_time.strftime('%Y-%m-%d')
        elif icfs_generated_by_ads_off_month_cohort_val > 1e-9: # If this cohort generates ICFs
        ...
        else: # This month generates 0 ICFs, but somehow needed from it (should not happen often)
            ads_off_date_str_calc_val = ads_off_month_period_val_calc.start_time.strftime('%Y-%m-%d')


    # --- Site Level Projections (AI) ---
    ai_site_proj_df = pd.DataFrame() # Initialize
    if all_sites_list_ai.size > 0:
        site_data_coll_ai_final_val = {site: {} for site in all_sites_list_ai}
        for site_n_final_init_val in all_sites_list_ai:
            for month_p_final_init_val in projection_calc_months: # Use overall calculation window
                ...
                site_data_coll_ai_final_val[site_n_final_init_val][(month_str_final_init_val, 'Projected ICFs Landed')] = 0.0 # Initialize as float

        for gen_month_site_idx_val in ai_gen_df.index: # Iterate through generation months
            qlof_for_month_val = site_level_monthly_qlof.get(gen_month_site_idx_val, {}) # Get QLs allocated to sites for this gen month
            for site_n_final_val in all_sites_list_ai:
                qls_for_site_this_gen_month_val = round(qlof_for_month_val.get(site_n_final_val, 0))
                # Assign QLs to their generation month for this site
                ...
                # Calculate ICFs generated by this site for this generation month
                ...
                site_pof_icf_rate_final_val = overall_pof_to_icf_rate # Default to overall rate
                ...
                if site_gen_icfs_this_gen_month_float > 0:
                    # Use overall avg_overall_lag_days for site ICF landing (can be refined later if site-specific lags are robustly available and desired here)
                    s_full_lag_m_val = int(np.floor(avg_overall_lag_days / days_in_avg_m))
                    s_rem_lag_d_val = avg_overall_lag_days - (s_full_lag_m_val * days_in_avg_m)
                    s_frac_next_val = s_rem_lag_d_val / days_in_avg_m; s_frac_curr_val = 1.0 - s_frac_next_val

                    s_land_m1_val = gen_month_site_idx_val + s_full_lag_m_val
                    s_land_m2_val = s_land_m1_val + 1

                    if s_land_m1_val in projection_calc_months: # Check if landing month is within display window
                        key_landed_m1_site = (s_land_m1_val.strftime('%Y-%m'), 'Projected ICFs Landed')
                        site_data_coll_ai_final_val[site_n_final_val][key_landed_m1_site] = \
                            site_data_coll_ai_final_val[site_n_final_val].get(key_landed_m1_site, 0.0) + (site_gen_icfs_this_gen_month_float * s_frac_curr_val)
                    if s_land_m2_val in projection_calc_months:
                        key_landed_m2_site = (s_land_m2_val.strftime('%Y-%m'), 'Projected ICFs Landed')
                        site_data_coll_ai_final_val[site_n_final_val][key_landed_m2_site] = \
                            site_data_coll_ai_final_val[site_n_final_val].get(key_landed_m2_site, 0.0) + (site_gen_icfs_this_gen_month_float * s_frac_next_val)
        ...
        if site_data_coll_ai_final_val:
            ...
            if not ai_site_proj_df.empty:
                ...
                ai_site_proj_df = ai_site_proj_df.sort_index(axis=1, level=[0,1]) # Sort columns for consistent display
                ...
                ai_site_proj_df = ai_site_proj_df.fillna(0) # Fill any NaNs that might have crept in
                # Add Grand Total row for site projections
                if not ai_site_proj_df.empty:
    ...
    # --- Feasibility Assessment & Messaging ---
    ...
    actual_lpi_month_achieved_this_run = current_goal_lpi_month_period # Default to goal LPI for the current run

    if not ai_results_df.empty and 'Cumulative_ICF_Landed' in ai_results_df:
        # Check if the *current run's ICF goal* is met by the *current run's LPI month*
        met_goal_series_val = ai_results_df[ai_results_df['Cumulative_ICF_Landed'] >= (current_goal_icf_number - 1e-9)] # Compare with float precision
        if not met_goal_series_val.empty:
            actual_lpi_month_achieved_this_run = met_goal_series_val.index.min() # Earliest month goal is met or exceeded
            if actual_lpi_month_achieved_this_run <= current_goal_lpi_month_period :
                 goal_met_on_time_this_run = True # Landed ICFs met/exceeded goal by target LPI

    # Strict unfeasibility check
    significant_icfs_still_to_assign_from_gen_planning = icfs_still_to_assign_globally > 0.1 # If >0.1 ICFs worth of QLs were not even planned
    ...
    # Nuanced Feasibility Messaging
    feasibility_prefix = "AI Projection: "
    detailed_outcome_message = ""

    # Goal description for message depends on run_mode
    ...
    if effective_icf_goal_for_run_msg > 0:
        landed_percentage_of_effective_goal = final_achieved_icfs_landed_run / effective_icf_goal_for_run_msg

    near_miss_threshold_pct = 0.95 # Landed at least 95% of ICF goal

    if not is_unfeasible_this_run: # Strictly feasible
        detailed_outcome_message = f"{goal_desc_for_msg} appear ACHIEVABLE."
        if final_achieved_icfs_landed_run > effective_icf_goal_for_run_msg:
            detailed_outcome_message += f" (Projected to exceed goal, landing {final_achieved_icfs_landed_run:.0f} ICFs)."
        elif run_mode == "best_case_extended_lpi": # Best case and feasible
             detailed_outcome_message = f"{goal_desc_for_msg}: Target of {effective_icf_goal_for_run_msg} ICFs ACHIEVED by {actual_lpi_month_achieved_this_run.strftime('%Y-%m')}."
    else: # Unfeasible by strict rules, provide detailed explanation
        ...
        if total_unallocated_qls_run > 0:
            constraint_msgs_list.append(f"{total_unallocated_qls_run:.0f} QLs unallocatable due to site caps.")
            hard_constraints_hit_flag = True
        if significant_icfs_still_to_assign_from_gen_planning:
            constraint_msgs_list.append(f"{icfs_still_to_assign_globally:.1f} ICFs (target: {effective_icf_goal_for_run_msg}) could not be fully planned in generation phase.")
            hard_constraints_hit_flag = True

        if hard_constraints_hit_flag:
            detailed_outcome_message = base_unfeasible_msg_for_else + "appear UNFEASIBLE due to planning constraints: " + " ".join(constraint_msgs_list)
        elif not goal_met_on_time_this_run: # Not strictly feasible, and no hard gen/cap constraints, so it's about landed ICFs/LPI
            ...
            if landed_percentage_of_effective_goal >= near_miss_threshold_pct and actual_lpi_month_achieved_this_run <= effective_lpi_for_run_msg:
                # Near miss on landed ICFs but on time for LPI
                ...
                if generation_goal_met_or_exceeded_in_float and final_achieved_icfs_landed_run < effective_icf_goal_for_run_msg:
                    detailed_outcome_message += " (Note: Goal met in total ICFs generated; landed sum slightly short due to monthly rounding)."
            else: # Missed LPI, or significant shortfall even if on time
                if actual_lpi_month_achieved_this_run > effective_lpi_for_run_msg and final_achieved_icfs_landed_run >= effective_icf_goal_for_run_msg : # Met ICF goal but LPI was late
                     detailed_outcome_message = base_unfeasible_msg_for_else + f"LPI NOT MET. Goal of {effective_icf_goal_for_run_msg} ICFs achieved by {achieved_by_date_str_msg}, which is after target LPI of {target_lpi_str_msg}."
                else: # Did not meet ICF goal by LPI (either shortfall or LPI also missed)
                     detailed_outcome_message = base_unfeasible_msg_for_else + f"LANDING GOAL SHORTFALL/LPI MISSED. Projected {final_achieved_icfs_landed_run:.0f} ICFs ({landed_percentage_of_effective_goal*100:.1f}%) by {achieved_by_date_str_msg} (Target: {effective_icf_goal_for_run_msg} ICFs by {target_lpi_str_msg})."
        else: # Fallback for other unfeasibility if not caught (e.g., LPI met, but some non-significant unallocated QLs or gen remainder)
            detailed_outcome_message = base_unfeasible_msg_for_else + "appear UNFEASIBLE for other reasons."
            # Append minor details if not covered
            ...
            if icfs_still_to_assign_globally > 1e-5 and not significant_icfs_still_to_assign_from_gen_planning and not hard_constraints_hit_flag:
                 detailed_outcome_message += f" Small remainder of {icfs_still_to_assign_globally:.2f} ICFs from generation."

    feasibility_msg_final_display = feasibility_prefix + detailed_outcome_message.strip()

    # Trim ai_results_df for display: show up to LPI achieved + a few months, or full projection window
    display_end_month_final_val = projection_calc_months[-1] if not projection_calc_months.empty else proj_start_month_period
    if not ai_results_df.empty and 'Cumulative_ICF_Landed' in ai_results_df:
        # Find month where current run's goal is met
        ...
        if not met_goal_series_trim_val.empty:
            ...
            try: # Add a buffer of 3 months after goal achievement for display
                ...
                # Display up to the later of (LPI_achieved + buffer) or (current_goal_LPI_month + buffer), capped by total horizon
            ...
            except Exception: pass # Keep original display_end_month_final_val on error

    ai_results_df_final_display_val = pd.DataFrame() # Initialize
    if not ai_results_df.empty and proj_start_month_period <= display_end_month_final_val:
        try:
            ai_results_df_final_display_val = ai_results_df.loc[proj_start_month_period:display_end_month_final_val].copy()
        except Exception: # Fallback if slicing fails
            ai_results_df_final_display_val = ai_results_df.copy()
    ...
    if 'st' in globals() and hasattr(st, 'session_state'):
        if run_mode == "primary":
            ...
            st.session_state.ai_results_df_debug_primary = ai_results_df.copy() # Store full results before trim
        elif run_mode == "best_case_extended_lpi":
...
with st.sidebar:
    ...
    with st.expander("Site Scoring Weights"):
        ...
        else: weights_normalized = {k: 0 for k in weights_input_local} # Avoid division by zero
    ...
    with st.expander("Projection Assumptions (Main Projections Tab)", expanded=False):
        ...
        if current_spend_df_rows != proj_horizon_editor_val or 'proj_spend_df_cache' not in st.session_state or \
           (not st.session_state.proj_spend_df_cache.empty and pd.Period(st.session_state.proj_spend_df_cache['Month'].iloc[0], freq='M') != future_months_ui_for_editor[0]): # Check if months align
        ...
        if 'Month' in edited_spend_df.columns and 'Planned_Spend' in edited_spend_df.columns:
             for index, row_spend in edited_spend_df.iterrows():
                 ...
                 except Exception: pass # Ignore errors in spend parsing for robustness
        ...
        if current_spend_df_rows != proj_horizon_editor_val or 'proj_cpqr_df_cache' not in st.session_state or \
            (not st.session_state.proj_cpqr_df_cache.empty and pd.Period(st.session_state.proj_cpqr_df_cache['Month'].iloc[0], freq='M') != future_months_ui_for_editor[0]): # Check if months align
        ...
        if 'Month' in edited_cpqr_df.columns and 'Assumed_CPQR' in edited_cpqr_df.columns:
            for index, row_cpqr in edited_cpqr_df.iterrows():
                try:
                    ...
                    if cpqr_val <=0: cpqr_val = default_cpqr_value # Ensure positive CPQR
        ...
        if use_rolling_flag_sidebar:
            ...
            if st.session_state.data_processed_successfully and st.session_state.referral_data_processed is not None and \
               st.session_state.ordered_stages is not None and st.session_state.ts_col_map is not None:
                # Display rolling rate log in sidebar if this option is chosen
                determine_effective_projection_rates(
                    ...
                    st.session_state.get('inter_stage_lags', {}), sidebar_display_area=st.sidebar) # Pass sidebar area
            else: st.sidebar.caption("Upload data to view calculated rolling rates.")
        else: rolling_window_months_sidebar = 0; st.sidebar.caption("Using manually input rates above for Projections tab.") # Reset window if manual
    st.divider()
...
if uploaded_referral_file is not None and uploaded_funnel_def_file is not None:
    if not st.session_state.data_processed_successfully: # Process only once or if files change (implicit with uploader key)
        ...
        if funnel_definition and ordered_stages and ts_col_map:
            ...
            try:
                 ...
                 # Attempt UTF-8, fallback to Latin-1 with replacement for problematic characters
                 ...
                 try:
                      # Warn about bad lines instead of erroring out immediately for CSV parsing
                      ...
                      if st.session_state.referral_data_processed is not None and not st.session_state.referral_data_processed.empty:
                           ...
                           # Calculate these once after successful processing
...
if st.session_state.data_processed_successfully:
    ...
    if "success_message_shown" not in st.session_state and referral_data_processed is not None: # Show success once
    ...
    with tab2:
        ...
        if referral_data_processed is not None and ordered_stages is not None and ts_col_map is not None and weights_normalized and not site_metrics_calculated_data.empty:
            ...
            display_cols_sites_exist_tab2 = [col for col in display_cols_sites_tab2 if col in ranked_sites_df_tab2.columns] # Only show existing columns
            ...
            if not final_ranked_display_tab2.empty:
                ...
                for col_fmt in final_ranked_display_tab2.columns:
                    if '%' in col_fmt and final_ranked_display_tab2[col_fmt].dtype == 'float': # Check if it's a rate column
                        ...
                        final_ranked_display_tab2[col_fmt] = final_ranked_display_tab2[col_fmt].apply(lambda x: f"{x:.1f}" if pd.notna(x) else '-')
                         ...
                         final_ranked_display_tab2[col_fmt] = final_ranked_display_tab2[col_fmt].apply(lambda x: f"{int(x):,}" if pd.notna(x) and x==x else '-') # x==x to handle NaNs correctly before int()
    ...
    with tab3:
        ...
        # Determine effective rates for projections tab (log displayed in sidebar if rolling is selected there)
        effective_proj_rates_tab3, method_desc_tab3 = determine_effective_projection_rates(
            referral_data_processed, ordered_stages, ts_col_map, rate_assumption_method_sidebar, # Uses sidebar choice
            rolling_window_months_sidebar, manual_proj_conv_rates_sidebar, inter_stage_lags_data,
            sidebar_display_area=None) # No specific display area needed here for the main tab
        ...
        with st.expander("View Calculated Average Inter-Stage Lags & Maturity Periods Used (for Rolling Rates)"):
            if inter_stage_lags_data:
                lag_df_list_tab3 = []
                # Recalculate maturity periods for display based on current lags and projection parameters
                ...
                if inter_stage_lags_data: # Check again, as it might be empty dict
                    for r_key_disp_tab3 in manual_proj_conv_rates_sidebar.keys(): # Iterate over defined projection rates
        ...
        if projection_results_df_tab3 is not None and not projection_results_df_tab3.empty:
            ...
            results_display_tab3 = projection_results_df_tab3.copy() # Work with a copy for display modifications

            # Create combined CPICF column
            ...
            elif 'Projected_CPICF_Cohort_Source_Mean' in results_display_tab3.columns: # Fallback if Low/High not present
            ...
            else: final_display_cols_proj_tab3 = base_display_cols_proj_tab3 # No CPICF data
            ...
            if not results_display_filtered_tab3.empty:
                results_display_filtered_tab3.index = results_display_filtered_tab3.index.strftime('%Y-%m') # Format index
                ...
                if 'Projected_ICF_Landed' in projection_results_df_tab3.columns:
                     ...
                     if isinstance(chart_data_tab3.index, pd.PeriodIndex): chart_data_tab3.index = chart_data_tab3.index.to_timestamp() # Convert PeriodIndex for chart
        ...
        if site_level_proj_tab3 is not None and not site_level_proj_tab3.empty:
            ...
            # Ensure 'Site' is a column for editing, handle if it's index
            if df_for_editor_tab3.index.name == 'Site': df_for_editor_tab3.reset_index(inplace=True)

            grand_total_row_tab3 = None # Extract Grand Total row to display separately (non-editable)
            ...
            elif "Grand Total" in df_for_editor_tab3.index: # If Grand Total is an index name
            ...
            # Flatten MultiIndex columns for data_editor if they exist
            if isinstance(df_for_editor_tab3.columns, pd.MultiIndex):
                df_for_editor_tab3.columns = [f"{col[1]} ({col[0]})" for col in df_for_editor_tab3.columns] # Metric (Month)
            ...
            edited_site_level_df_tab3 = st.data_editor(df_for_editor_tab3, use_container_width=True, key="site_level_editor_v3_tab3", num_rows="dynamic") # Allow adding/deleting rows if needed
            ...
            if grand_total_row_tab3 is not None and not grand_total_row_tab3.empty:
                ...
                # Ensure 'Site' column for display if it was index
                if 'Site' not in grand_total_row_tab3_display.columns:
                    if grand_total_row_tab3_display.index.name == 'Site' or "Grand Total" in grand_total_row_tab3_display.index:
                         ...
                         if grand_total_row_tab3_display.columns[0] == 'index': # Rename if reset_index creates 'index'
                             grand_total_row_tab3_display.rename(columns={'index':'Site'}, inplace=True)
                if 'Site' in grand_total_row_tab3_display.columns: # Order 'Site' first
                ...
                formatters_gt_tab3 = {col: "{:,.0f}" for col in numeric_cols_gt_tab3} # Format numbers
    ...
    with tab_ai:
        ...
        selected_rate_method_label_ai_tab = st.radio("Base AI Forecast Conversion Rates On:", options=list(rate_options_ai_display.keys()), index=4, key="ai_rate_method_radio_v3", horizontal=True) # Default to 3-month rolling
        ai_rate_assumption_method_internal_val = "Manual Input Below"; ai_rolling_window_months_internal_val = 0 # Defaults for manual
        if selected_rate_method_label_ai_tab == "Overall Historical Average": ai_rate_assumption_method_internal_val = "Rolling Historical Average"; ai_rolling_window_months_internal_val = 999 # 999 for overall
            ...
            except: ai_rolling_window_months_internal_val = 3 # Fallback if parsing fails
        ...
        if not site_metrics_calculated_data.empty and 'Site' in site_metrics_calculated_data.columns and referral_data_processed is not None:
            ...
            if ts_pof_col_for_avg_calc_val and ts_pof_col_for_avg_calc_val in referral_data_processed.columns and 'Site' in referral_data_processed.columns:
                ...
                # Ensure Submission_Month is PeriodDtype for groupby
                if not pd.api.types.is_period_dtype(temp_df_for_avg_val['Submission_Month']):
                    try: temp_df_for_avg_val['Submission_Month'] = temp_df_for_avg_val['Submission_Month'].dt.to_period('M')
                    except AttributeError: # If already string or other type that can't use .dt
                        ...
                        except: pass # If conversion fails, proceed without avg
            ...
            for site_name_cap_iter_val in site_metrics_calculated_data['Site'].unique():
                site_cap_editor_data_list.append({ "Site": site_name_cap_iter_val, "Historical Avg. Monthly POF": avg_monthly_ql_per_site_val.get(site_name_cap_iter_val, 0), "Monthly POF Cap": np.nan }) # Default cap to NaN (no cap)
            ...
            if site_cap_editor_data_list:
                ...
                edited_site_caps_df_ai_val = st.data_editor( pd.DataFrame(site_cap_editor_data_list), key="ai_site_caps_editor_v3", use_container_width=True,
                    ...
                    num_rows="dynamic" # Allow adding/removing sites if user has a list beyond what's in site_metrics
                )
        ...
        if st.button("🚀 Generate AI Forecast", key="run_ai_forecast_v5"):
            st.session_state.ai_forecast_results = None # Clear previous results
            # Clear debug states
            ...
            if selected_rate_method_label_ai_tab == "Manual Input Below":
                ...
                ai_effective_rates, ai_rates_method_desc = determine_effective_projection_rates(
                    ...
                    ai_rolling_window_months_internal_val, manual_proj_conv_rates_sidebar, # Pass sidebar manual as fallback base
                    inter_stage_lags_data, sidebar_display_area=None ) # No sidebar display needed here
                if "Error" in ai_rates_method_desc or "Failed" in ai_rates_method_desc or "No History" in ai_rates_method_desc or not ai_effective_rates or all(v == 0 for v in ai_effective_rates.values()):
                    ...
                    ai_effective_rates = manual_proj_conv_rates_sidebar # Fallback to general manual rates
                    ai_rates_method_desc = f"Manual (Fallback from Projections Tab Sidebar due to issue with '{selected_rate_method_label_ai_tab}')"

            # Determine POF to ICF lag for AI forecast
            ...
            avg_lag_to_use_for_ai_val = avg_pof_icf_lag_ai_val
            lag_source_message_ai_val = f"Calculated Historical POF-ICF lag: {avg_pof_icf_lag_ai_val:.1f} days" if pd.notna(avg_pof_icf_lag_ai_val) else "POF-ICF lag calculation failed."
            if pd.isna(avg_lag_to_use_for_ai_val): # Fallback if lag calculation fails
                avg_lag_to_use_for_ai_val = 30.0 # Default lag
                lag_source_message_ai_val = f"Using default POF-ICF lag: {avg_lag_to_use_for_ai_val:.1f} days (direct calculation failed or not enough data)"

            if pd.isna(avg_lag_to_use_for_ai_val): st.error("Cannot run AI Forecast: Average POF to ICF lag is not available or could not be determined.")
                ...
                ai_results_df_run1, ai_site_df_run1, ai_ads_off_run1, ai_message_run1, ai_unfeasible_run1, ai_actual_icfs_run1 = calculate_ai_forecast_core(
                    ...
                    effective_projection_conv_rates=ai_effective_rates, avg_overall_lag_days=avg_lag_to_use_for_ai_val,
                ...
                if 'st' in globals() and hasattr(st, 'sidebar') and hasattr(st.session_state, 'ai_gen_df_debug_primary'): # Check if debug data exists
                ...
                if ai_unfeasible_run1: # If primary run is unfeasible by strict rules
                    ...
                    ai_results_df_run2, ai_site_df_run2, ai_ads_off_run2, ai_message_run2, ai_unfeasible_run2, ai_actual_icfs_run2 = calculate_ai_forecast_core(
                        goal_lpi_date_dt_orig=ai_goal_lpi_date, # Original LPI goal for context
                        goal_icf_number_orig=ai_goal_icf_num,   # Original ICF goal
                        estimated_cpql_user=ai_cpql_estimate, icf_variation_percent=proj_icf_variation_percent_sidebar,
                        processed_df=referral_data_processed, ordered_stages=ordered_stages, ts_col_map=ts_col_map,
                        effective_projection_conv_rates=ai_effective_rates, avg_overall_lag_days=avg_lag_to_use_for_ai_val,
                        site_metrics_df=site_metrics_calculated_data, projection_horizon_months=proj_horizon_sidebar,
                        site_caps_input=default_site_caps_ai_input_val, site_scoring_weights_for_ai=weights_normalized,
                        cpql_inflation_factor_pct=ai_cpql_inflation_factor_sidebar, ql_vol_increase_threshold_pct=ai_ql_volume_threshold_sidebar,
                        run_mode=run_mode_for_call_best_case_val, # This will adjust LPI internally
                        ai_monthly_ql_capacity_multiplier=ai_monthly_ql_capacity_multiplier_sidebar_val
                    )
                    ...
                    if 'st' in globals() and hasattr(st, 'sidebar') and hasattr(st.session_state, 'ai_gen_df_debug_best_case'):
                        ...
                        st.sidebar.markdown(f"**Unfeasible (Best Case - by strict rule for extended LPI):** `{ai_unfeasible_run2}`")
                    ...
                    # Store results of the best-case run if it was executed
                    st.session_state.ai_forecast_results = {
        ...
        if st.session_state.get('ai_forecast_results'):
            ...
            ai_unfeasible_display_flag = results_ai_tab['unfeasible'] # Strict unfeasibility flag
            ai_actual_icfs_display = results_ai_tab['actual_icfs']
            run_mode_msg_display = results_ai_tab.get('run_mode_displayed', 'primary')

            st.markdown("---")
            # Use the nuanced message for display; color based on the strict unfeasibility flag
            if not ai_unfeasible_display_flag : # Strictly feasible
                 st.success(f"Forecast Status: {ai_message_display}")
            elif "NEAR MISS" in ai_message_display: # If message indicates a near miss, use warning color
                st.warning(f"Feasibility Note: {ai_message_display}")
            else: # Otherwise, still unfeasible
            ...
            # If the message indicates a shortfall (even if primary run was unfeasible but best_case is shown), adjust display
            ...
            elif ai_message_display and "Not Calculated" not in ai_message_display:
