...
def calculate_site_metrics(_processed_df, ordered_stages, ts_col_map):
    ...
    projection_segments_for_site_lag = [
        (stage_pof_name, stage_psa_name),
        (stage_psa_name, stage_sts_name),
        (stage_sts_name, stage_appt_name),
        (stage_appt_name, stage_icf_name)
    ]
    try:
...
def score_sites(_site_metrics_df, weights):
    ...
    try:
        ...
        for col in metrics_to_scale:
            if col not in scaled_metrics_data.columns: scaled_metrics_data[col] = 0 if col not in lower_is_better else np.nan
            if col in lower_is_better:
                max_val = scaled_metrics_data[col].max(); fill_val = (max_val + 1) if pd.notna(max_val) and max_val > 0 else 999
                scaled_metrics_data[col] = scaled_metrics_data[col].fillna(fill_val)
            else: scaled_metrics_data[col] = scaled_metrics_data[col].fillna(0)
            ...
            for col in metrics_to_scale:
                 if col in scaled_metrics_data.columns:
                     ...
                     if min_val == max_val: scaled_metrics_display[col] = 0.5
                         ...
                         scaler = MinMaxScaler(); scaled_values = scaler.fit_transform(scaled_metrics_data[[col]]); scaled_metrics_display[col] = scaled_values.flatten()
        ...
        for metric, weight_pct in weights.items():
             ...
             if metric in scaled_metrics_display.columns:
                 ...
                 if current_scaled_metric_series is not None: site_metrics_df_indexed['Score_Raw'] += current_scaled_metric_series.fillna(0.5) * weight
                 else: site_metrics_df_indexed['Score_Raw'] += 0.5 * weight
...
def determine_effective_projection_rates(_processed_df, ordered_stages, ts_col_map,
    ...
    try:
        ...
        for rate_key in manual_rates_sidebar.keys():
            ...
            except ValueError: calculated_rolling_rates[rate_key] = manual_rates_sidebar.get(rate_key, 0.0); substitutions_made_log.append(f"{rate_key}: Error parsing names, used manual."); continue
            ...
            if actual_col_from in hist_counts.columns and col_to_cleaned_name in hist_counts.columns:
                        ...
                        if denominator_val < MIN_DENOMINATOR_FOR_RATE_CALC:
                            ...
                            if is_manual_rate_placeholder:
                                if pd.notna(overall_hist_rate_for_key) and total_denominator >= MIN_TOTAL_DENOMINATOR_FOR_OVERALL_RATE:
                                    rate_for_this_month = overall_hist_rate_for_key; log_reason_detail = f"manual placeholder, used overall hist. ({overall_hist_rate_for_key*100:.1f}%, total N={total_denominator})"
                ...
                if not adjusted_monthly_rates_list: calculated_rolling_rates[rate_key] = manual_rates_sidebar.get(rate_key, 0.0); substitutions_made_log.append(f"{rate_key}: No mature hist. mths (mat: {maturity_days_for_this_rate}d), used manual."); continue
                ...
                if actual_window_calc > 0 and not adjusted_monthly_rates_series.empty:
                    ...
                    if not rolling_avg_rate.empty:
                        latest_rolling_rate = rolling_avg_rate.iloc[-1]; calculated_rolling_rates[rate_key] = latest_rolling_rate if pd.notna(latest_rolling_rate) else 0.0; valid_historical_rates_found = True
                    ...
                    if not adjusted_monthly_rates_series.empty:
                        mean_mature_rate = adjusted_monthly_rates_series.mean(); calculated_rolling_rates[rate_key] = mean_mature_rate if pd.notna(mean_mature_rate) else manual_rates_sidebar.get(rate_key, 0.0)
        ...
        if sidebar_display_area and substitutions_made_log:
            with sidebar_display_area.expander("Rolling Rate Calculation Log (Adjustments & Maturity)", expanded=False):
                ...
                for log_entry in substitutions_made_log: st.caption(log_entry) # Changed from st.sidebar.caption
...
# --- MODIFIED: calculate_projections function ---
@st.cache_data
def calculate_projections(_processed_df, ordered_stages, ts_col_map, projection_inputs):
    ...
    # --- MODIFIED: Add 'icf_variation_percentage' to required keys ---
    ...
    # --- NEW: Get ICF variation percentage ---
    ...
    projection_segments_for_lag = [("Passed Online Form", "Pre-Screening Activities"), ("Pre-Screening Activities", "Sent To Site"), ("Sent To Site", "Appointment Scheduled"), ("Appointment Scheduled", "Signed ICF")]
    ...
    if inter_stage_lags:
        ...
        if all_segments_found_and_valid and valid_segments_count == len(projection_segments_for_lag):
            avg_actual_lag_days_for_display = calculated_sum_of_lags; lag_calculation_method_message = "Using summed inter-stage lags for projection path."
        else: all_segments_found_and_valid = False; lag_calculation_method_message = "Summed inter-stage lag failed. "
    ...
    if not all_segments_found_and_valid or pd.isna(avg_actual_lag_days_for_display):
        ...
        if start_stage_for_overall_lag:
            ...
            if ts_col_start_overall and ts_col_end_overall and ts_col_start_overall in processed_df.columns and ts_col_end_overall in processed_df.columns:
                overall_lag_calc_val = calculate_avg_lag_generic(processed_df, ts_col_start_overall, ts_col_end_overall)
        if pd.notna(overall_lag_calc_val): avg_actual_lag_days_for_display = overall_lag_calc_val; lag_calculation_method_message += "Used historical overall lag (first funnel stage to ICF)."
    ...
    try:
        ...
        icf_stage_name_proj = "Signed ICF"; current_proj_count_col = 'Forecasted_PSQ'; icf_proj_col_base = "" # Renamed for clarity
        for stage_from, stage_to in projection_segments_for_lag:
            ...
            if stage_to == icf_stage_name_proj: icf_proj_col_base = proj_col_to; break
        projection_results = pd.DataFrame(index=future_months); projection_results['Projected_ICF_Landed'] = 0.0
        if not icf_proj_col_base or icf_proj_col_base not in projection_cohorts.columns:
            st.error(f"Critical Error: Projected ICF column ('{icf_proj_col_base}') not found."); return default_return_tuple[0], default_return_tuple[1], default_return_tuple[2], default_return_tuple[3], default_return_tuple[4], "ICF Proj Col Missing"

        # --- NEW: Calculate ICF range for the cohort ---
        icf_proj_col_low = f"{icf_proj_col_base}_low"
        icf_proj_col_high = f"{icf_proj_col_base}_high"
        projection_cohorts[icf_proj_col_low] = (projection_cohorts[icf_proj_col_base] * (1 - variation_factor)).round(0).astype(int).clip(lower=0)
        projection_cohorts[icf_proj_col_high] = (projection_cohorts[icf_proj_col_base] * (1 + variation_factor)).round(0).astype(int).clip(lower=0)

        # --- MODIFIED: Calculate CPICF mean, low, and high for the cohort ---
        projection_cohorts['Projected_CPICF_Cohort_Mean'] = (projection_cohorts['Forecasted_Ad_Spend'] / projection_cohorts[icf_proj_col_base].replace(0, np.nan)).round(2)
        projection_cohorts['Projected_CPICF_Cohort_Low'] = (projection_cohorts['Forecasted_Ad_Spend'] / projection_cohorts[icf_proj_col_high].replace(0, np.nan)).round(2) # High ICFs -> Low CPICF
        projection_cohorts['Projected_CPICF_Cohort_High'] = (projection_cohorts['Forecasted_Ad_Spend'] / projection_cohorts[icf_proj_col_low].replace(0, np.nan)).round(2)  # Low ICFs -> High CPICF
        ...
        for start_month_period in projection_cohorts.index:
            icfs_from_this_cohort = projection_cohorts.loc[start_month_period, icf_proj_col_base] # Use base ICF for landing distribution
            if icfs_from_this_cohort == 0: continue
            full_lag_months = int(np.floor(overall_current_lag_days_to_use / days_in_avg_month))
            remaining_lag_days_component = overall_current_lag_days_to_use - (full_lag_months * days_in_avg_month)
            fraction_for_next_month = remaining_lag_days_component / days_in_avg_month; fraction_for_current_offset_month = 1.0 - fraction_for_next_month
            icfs_month_1 = icfs_from_this_cohort * fraction_for_current_offset_month; icfs_month_2 = icfs_from_this_cohort * fraction_for_next_month
            landing_month_1_period = start_month_period + full_lag_months; landing_month_2_period = start_month_period + full_lag_months + 1
        ...
        projection_cohorts['Cumulative_Projected_ICF_Generated'] = projection_cohorts[icf_proj_col_base].cumsum() # Use base ICF for cumulative generation
        ...
        # --- MODIFIED: Prepare CPICF low, mean, high for display_df ---
        cpicf_mean_series = projection_cohorts['Projected_CPICF_Cohort_Mean']
        cpicf_low_series = projection_cohorts['Projected_CPICF_Cohort_Low']
        cpicf_high_series = projection_cohorts['Projected_CPICF_Cohort_High']

        cpicf_display_mean = pd.Series(index=future_months, dtype=float)
        cpicf_display_low = pd.Series(index=future_months, dtype=float)
        cpicf_display_high = pd.Series(index=future_months, dtype=float)

        lag_for_cpicf_display = int(np.round(overall_current_lag_days_to_use / 30.4375))
        for i_cohort, cohort_start_month in enumerate(projection_cohorts.index):
            ...
            if primary_land_m in cpicf_display_mean.index:
                if pd.isna(cpicf_display_mean.loc[primary_land_m]): # Land only if not already filled
                    cpicf_display_mean.loc[primary_land_m] = cpicf_mean_series.iloc[i_cohort]
                    cpicf_display_low.loc[primary_land_m] = cpicf_low_series.iloc[i_cohort]
                    cpicf_display_high.loc[primary_land_m] = cpicf_high_series.iloc[i_cohort]

        display_df['Projected_CPICF_Cohort_Source_Mean'] = cpicf_display_mean
        display_df['Projected_CPICF_Cohort_Source_Low'] = cpicf_display_low
        display_df['Projected_CPICF_Cohort_Source_High'] = cpicf_display_high
        # --- End MODIFIED CPICF display prep ---
        ...
        if 'Site' in _processed_df.columns and not _processed_df['Site'].empty and ordered_stages:
            ...
            for start_month_period in future_months:
                ...
                for site_name in all_sites:
                    ...
                    site_proj_pof_cohort = total_psq_this_cohort * site_prop
                    ...
                    for i_seg, (stage_from_seg, stage_to_seg) in enumerate(projection_segments_for_lag):
                        site_rate_key = f"{stage_from_seg} -> {stage_to_seg} %"
                        if stage_from_seg == "Passed Online Form" and stage_to_seg == "Pre-Screening Activities": site_rate_key = 'POF -> PSA %'
                        elif stage_from_seg == "Pre-Screening Activities" and stage_to_seg == "Sent To Site": site_rate_key = 'PSA -> StS %'
                        elif stage_from_seg == "Sent To Site" and stage_to_seg == "Appointment Scheduled": site_rate_key = 'StS -> Appt %'
                        elif stage_from_seg == "Appointment Scheduled" and stage_to_seg == "Signed ICF": site_rate_key = 'Appt -> ICF %'
                        overall_rate_key = f"{stage_from_seg} -> {stage_to_seg}"
                        rate_to_use = final_projection_conv_rates.get(overall_rate_key, 0.0)
                        if not site_perf_row.empty and site_rate_key in site_perf_row.columns:
                            ...
                            if pd.notna(site_specific_rate_val): rate_to_use = site_specific_rate_val
                        current_site_proj_count *= rate_to_use
                        if stage_to_seg == "Signed ICF": break
                    site_proj_icfs_generated_this_cohort = current_site_proj_count
                    lag_to_use_for_site = overall_current_lag_days_to_use
                    if not site_perf_row.empty and 'Site Projection Lag (Days)' in site_perf_row.columns:
                        site_specific_lag_val = site_perf_row['Site Projection Lag (Days)'].iloc[0]
                        if pd.notna(site_specific_lag_val):
                            lag_to_use_for_site = site_specific_lag_val
                    if site_proj_icfs_generated_this_cohort > 0:
                        full_lag_m_site = int(np.floor(lag_to_use_for_site / days_in_avg_month))
                        remain_lag_days_comp_site = lag_to_use_for_site - (full_lag_m_site * days_in_avg_month)
            ...
            if site_data_collector:
                ...
                if not site_level_projections_df_temp.empty:
                    ...
                    if not site_level_projections_df.empty:
                        numeric_cols_site_level = [col for col in site_level_projections_df.columns if pd.api.types.is_numeric_dtype(site_level_projections_df[col])]
...
ad_spend_input_dict = {}; weights_normalized = {}; proj_horizon_sidebar = 12
...
# --- NEW: Initialize sidebar variable for ICF variation ---
proj_icf_variation_percent_sidebar = 10 # Default to 10%
...
with st.sidebar:
    ...
    with st.expander("Site Scoring Weights"):
        ...
        weights_input_local["StS -> Appt %"] = st.slider("StS -> Appt Sched %", 0, 100, 30, key='w_sa_site_score')
        weights_input_local["Appt -> ICF %"] = st.slider("Appt Sched -> ICF %", 0, 100, 15, key='w_ai_site_score')
        weights_input_local["Lag Qual -> ICF (Days)"] = st.slider("Lag Qual (POF) -> ICF (Days)", 0, 100, 0, key='w_lagqicf')
        weights_input_local["Site Projection Lag (Days)"] = st.slider("Site Projection Lag (Days)", 0, 100, 0, key='w_siteprojlag')
    ...
    with st.expander("Projection Assumptions", expanded=True):
            ...
            if st.session_state.data_processed_successfully and st.session_state.referral_data_processed is not None and \
               st.session_state.ordered_stages is not None and st.session_state.ts_col_map is not None:
                determine_effective_projection_rates(
                    ...
                    st.session_state.get('inter_stage_lags', {}), sidebar_display_area=st.sidebar) # Corrected st.sidebar usage
        ...
        # --- NEW: Sidebar input for ICF variation percentage ---
        st.markdown("---")
        st.subheader("Projection Sensitivity")
        proj_icf_variation_percent_sidebar = st.slider(
            "Projected ICF Variation (+/- %)",
            min_value=0, max_value=50, value=proj_icf_variation_percent_sidebar, step=1,
            key='proj_icf_variation_widget',
            help="This percentage variation is applied to the number of ICFs generated by each cohort to estimate a Low-Mean-High range for the Cohort CPICF."
        )
            ...
            try:
                 ...
                 try:
                      ...
                      if st.session_state.referral_data_processed is not None and not st.session_state.referral_data_processed.empty:
                           ...
                           st.session_state.inter_stage_lags = calculate_overall_inter_stage_lags(st.session_state.referral_data_processed, st.session_state.ordered_stages, st.session_state.ts_col_map)
...
if st.session_state.data_processed_successfully:
    referral_data_processed = st.session_state.referral_data_processed; funnel_definition = st.session_state.funnel_definition
    ordered_stages = st.session_state.ordered_stages; ts_col_map = st.session_state.ts_col_map
    ...
    st.markdown("---"); tab1, tab2, tab3 = st.tabs(["📅 Monthly ProForma", "🏆 Site Performance", "📈 Projections"])
    with tab1:
        ...
        if referral_data_processed is not None and ordered_stages is not None and ts_col_map is not None and ad_spend_input_dict is not None:
            ...
            if not proforma_df.empty:
                ...
                try: csv = proforma_df.reset_index().to_csv(index=False).encode('utf-8'); st.download_button(label="Download ProForma Data", data=csv, file_name='monthly_proforma.csv', mime='text/csv', key='dl_proforma')
        ...
        if referral_data_processed is not None and ordered_stages is not None and ts_col_map is not None and weights_normalized is not None:
            ...
            if not site_metrics_calculated.empty:
                ...
                display_cols_sites = ['Site', 'Score', 'Grade', 'Total Qualified',
                                      'PSA Count', 'StS Count', 'Appt Count', 'ICF Count',
                                      'Qual -> ICF %',
                ...
                if not final_ranked_display.empty:
                    ...
                    try: csv_sites = final_ranked_display.to_csv(index=False).encode('utf-8'); st.download_button(label="Download Site Ranking", data=csv_sites, file_name='site_ranking.csv', mime='text/csv', key='dl_sites')
                    except Exception as e: st.warning(f"Download button error: {e}")
    ...
    with tab3:
        ...
        _effective_projection_conv_rates_tab3, _method_desc_for_display_tab3 = determine_effective_projection_rates(
            ...
            rolling_window_months_sidebar, manual_proj_conv_rates_sidebar, inter_stage_lags_data, sidebar_display_area=None) # No sidebar area for main tab
        ...
        with st.expander("View Calculated Average Inter-Stage Lags & Maturity Periods Used"):
            if inter_stage_lags_data:
                lag_df_list = []; temp_maturity_periods_display = {}
                display_maturity_lag_multiplier = 1.5; display_min_effective_maturity = 20; display_default_maturity = 45
                if inter_stage_lags_data:
                ...
                for key, val in inter_stage_lags_data.items():
                    ...
                    lag_df_list.append({'Stage Transition': key, 'Avg Lag (Days)': f"{val:.1f}" if pd.notna(val) else "N/A", 'Implied Maturity Used (Days)': f"{maturity_p_display}" if isinstance(maturity_p_display, (int, float)) else maturity_p_display})
        ...
        st.markdown("---")
        # --- MODIFIED: Add icf_variation_percentage to projection_inputs ---
        projection_inputs = {
            ...
            'icf_variation_percentage': proj_icf_variation_percent_sidebar # NEW
        }
        projection_results_df, avg_lag_days_used_for_proj, lpi_date_str_proj, ads_off_date_str_proj, site_level_projections_df, lag_calc_msg_from_proj = calculate_projections(referral_data_processed, ordered_stages, ts_col_map, projection_inputs)
        st.markdown("---")
        ...
        if projection_results_df is not None and not projection_results_df.empty and isinstance(projection_results_df, pd.DataFrame):
            ...
            # --- MODIFIED: Columns to display and formatting for CPICF range ---
            cols_to_check_for_cpicf = ['Projected_CPICF_Cohort_Source_Low', 'Projected_CPICF_Cohort_Source_Mean', 'Projected_CPICF_Cohort_Source_High']
            base_display_cols_proj = ['Forecasted_Ad_Spend', 'Forecasted_Qual_Referrals', 'Projected_ICF_Landed']
            
            results_display = projection_results_df.copy() # Work with a copy
            
            # Create the combined CPICF display column if range data exists
            if all(c in results_display.columns for c in cols_to_check_for_cpicf):
                results_display['Projected CPICF (Low-Mean-High)'] = results_display.apply(
                    lambda row: (f"${row['Projected_CPICF_Cohort_Source_Low']:,.2f} - ${row['Projected_CPICF_Cohort_Source_Mean']:,.2f} - ${row['Projected_CPICF_Cohort_Source_High']:,.2f}"
                                ...
                                if proj_icf_variation_percent_sidebar > 0 # Only show range if variation > 0
                                else (f"${row['Projected_CPICF_Cohort_Source_Mean']:,.2f}" if pd.notna(row['Projected_CPICF_Cohort_Source_Mean']) else "-"),
                    axis=1
                )
                final_display_cols_proj = base_display_cols_proj + ['Projected CPICF (Low-Mean-High)']
            elif 'Projected_CPICF_Cohort_Source_Mean' in results_display.columns: # Fallback if only mean exists (e.g. old cache or error)
                results_display.rename(columns={'Projected_CPICF_Cohort_Source_Mean': 'Projected CPICF (Mean)'}, inplace=True)
                results_display['Projected CPICF (Mean)'] = results_display['Projected CPICF (Mean)'].apply(lambda x: f"${x:,.2f}" if pd.notna(x) else '-')
                final_display_cols_proj = base_display_cols_proj + ['Projected CPICF (Mean)']
            else: # Should not happen if calculate_projections runs correctly
                final_display_cols_proj = base_display_cols_proj
                st.warning("CPICF data columns missing in projection results.")

            results_display_filtered = results_display[[col for col in final_display_cols_proj if col in results_display.columns]].copy()

            if not results_display_filtered.empty:
                results_display_filtered.index = results_display_filtered.index.strftime('%Y-%m')
                if 'Forecasted_Ad_Spend' in results_display_filtered:
                    results_display_filtered['Forecasted_Ad_Spend'] = results_display_filtered['Forecasted_Ad_Spend'].apply(lambda x: f"${x:,.2f}" if pd.notna(x) else '-')
                if 'Forecasted_Qual_Referrals' in results_display_filtered:
                    results_display_filtered['Forecasted_Qual_Referrals'] = results_display_filtered['Forecasted_Qual_Referrals'].fillna(0).astype(int).map('{:,}'.format)
                if 'Projected_ICF_Landed' in results_display_filtered:
                    results_display_filtered['Projected_ICF_Landed'] = results_display_filtered['Projected_ICF_Landed'].fillna(0).astype(int).map('{:,}'.format)
                
                st.dataframe(results_display_filtered.style.format(na_rep='-'))

                if 'Projected_ICF_Landed' in projection_results_df.columns:
                     st.subheader("Projected ICFs Landed Over Time"); chart_data = projection_results_df[['Projected_ICF_Landed']].copy()
                     if isinstance(chart_data.index, pd.PeriodIndex): chart_data.index = chart_data.index.to_timestamp()
                     chart_data['Projected_ICF_Landed'] = pd.to_numeric(chart_data['Projected_ICF_Landed'], errors='coerce').fillna(0); st.line_chart(chart_data)
                try: csv_proj = results_display_filtered.reset_index().to_csv(index=False).encode('utf-8'); st.download_button(label="Download Projection Data", data=csv_proj, file_name='projection.csv', mime='text/csv', key='dl_proj')
                except Exception as e: st.warning(f"Download button error: {e}")
            else: st.warning("Projection results table is empty after selecting columns.")
        else: st.warning("Could not calculate projections.")
        st.markdown("---"); st.subheader("Site-Level Monthly Projections (Editable)")
        if site_level_projections_df is not None and not site_level_projections_df.empty:
            ...
            # Ensure 'Site' column is first for data_editor if it exists
            if 'Site' in editable_sites_df.columns:
                cols = ['Site'] + [col for col in editable_sites_df.columns if col != 'Site']
                editable_sites_df = editable_sites_df[cols]
            
            edited_site_level_df = st.data_editor(editable_sites_df, use_container_width=True, key="site_level_editor", num_rows="dynamic") # Consider num_rows="fixed" or "dynamic"
            ...
            try: csv_site_proj = edited_site_level_df.to_csv(index=False).encode('utf-8'); st.download_button(label="Download Edited Site Projections", data=csv_site_proj, file_name='edited_site_level_projections.csv', mime='text/csv', key='dl_edited_site_proj_main')
