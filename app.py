...
def calculate_site_metrics(_processed_df, ordered_stages, ts_col_map):
    ...
    try: 
        ...
        for site_name, group in site_groups:
            ...
            if not sent_to_site_group.empty and parsed_status_col in sent_to_site_group and parsed_stage_col in sent_to_site_group : # Check if parsed_stage_col exists
                for idx, row in sent_to_site_group.iterrows():
                    ...
                    stage_history_list = row.get(parsed_stage_col, []) # This might be None if column doesn't exist
...
def score_sites(_site_metrics_df, weights): # This is the function you provided from Turn 77
                ...
                site_metrics_df_indexed['Grade'] = pd.qcut(site_metrics_df_indexed['Score_Rank_Percentile'], q=bins, labels=labels, duplicates='drop') 
                 ...
                 def assign_grade_fallback(score_value): 
                     ...
                     if score_value >= 90: 
                         ...
                         return 'B'
                     elif score_value >= 70: # THIS IS THE LINE (approx 320 in your latest error)
                         return 'C'
                     elif score_value >= 60: 
                         return 'D'
                     else: 
...
def calculate_projections(_processed_df, ordered_stages, ts_col_map, projection_inputs): 
    ...
    # --- Lag Calculation ---
    start_stage_for_lag = ordered_stages[0] # e.g. "Passed Online Form"
    end_stage_for_lag = "Signed ICF"
    ts_col_start_lag = ts_col_map.get(start_stage_for_lag)
    ts_col_end_lag = ts_col_map.get(end_stage_for_lag)

    if ts_col_start_lag and ts_col_end_lag and \
       ts_col_start_lag in processed_df.columns and ts_col_end_lag in processed_df.columns and \
       pd.api.types.is_datetime64_any_dtype(processed_df[ts_col_start_lag]) and \
       pd.api.types.is_datetime64_any_dtype(processed_df[ts_col_end_lag]):
        
        valid_ts_df_overall = processed_df.dropna(subset=[ts_col_start_lag, ts_col_end_lag])
        if not valid_ts_df_overall.empty:
            time_diff_overall = valid_ts_df_overall[ts_col_end_lag] - valid_ts_df_overall[ts_col_start_lag]
    ...
    if pd.isna(avg_actual_lag_days_for_display):
        # Simplified fallback - check if full step-wise sum is really needed or if 30 days is OK
    ...
    try:
        ...
        lpi_date_str = "Goal Not Met"; ads_off_date_str = "N/A (Check Goal/Horizon)"
...
goal_icf_count_sidebar = 100 # Default goal, initialized before sidebar
...
with st.sidebar:
    ...
    with st.expander("Projection Assumptions", expanded=True): 
        ...
        goal_icf_count_sidebar = st.number_input("Goal Total ICFs", min_value=1, value=goal_icf_count_sidebar, step=1, key='goal_icf_input') # MODIFIED: Already present from user code
...
if st.session_state.data_processed_successfully:
    ...
    with tab3:
        ...
        projection_inputs = {
            ...
            'goal_icf': goal_icf_count_sidebar # ADDED: Pass goal_icf to projection_inputs
        }
        
        # UPDATED: Expect 4 values from calculate_projections now
        ...
        # --- ADDED: Display LPI and Ads Off Date ---
        st.markdown("---")
